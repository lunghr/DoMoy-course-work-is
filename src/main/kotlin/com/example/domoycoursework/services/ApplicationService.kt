...
//            ticketRepository.createApplication(applicationRequestDto, it.id, files)
...
//            ticketRepository.findApplicationById(id)?.let { application ->
...
//                }
//                ticketRepository.updateApplicationStatus(
//                    id,
//                    TicketStatus.valueOf(applicationResponseDto.status)
//                )
//                ticketCommentRepository.createApplicationResponse(
...
//        return ticketRepository.findApplicationById(id)?.let { application ->
...
//            responses = ticketCommentRepository.findAllResponsesByApplicationId(application.id)
//        )
//    }
//
...
//    }
//
//    fun getResponses(id: Int, token: String): List<TicketComment> {
//        return ticketRepository.findApplicationById(id)?.let { application ->
//            userService.loadUserByEmail(jwtService.getUsername(jwtService.extractToken(token)))?.let { user ->
//                if (application.userId != user.id) {
//                    throw UserNotFoundException("User is not the author of this application")
//                }
//                return ticketCommentRepository.findAllResponsesByApplicationId(application.id)
//            } ?: adminService.loadAdminByEmail(jwtService.getUsername(jwtService.extractToken(token)))?.let {
//                return ticketCommentRepository.findAllResponsesByApplicationId(application.id)
//            } ?: throw UserNotFoundException("User not found")
//        } ?: throw NotFoundException("Application not found")
...
//            return ticketRepository.findAllByUserId(user.id)
//        } ?: throw UserNotFoundException("User not found")
//    }
//
//}